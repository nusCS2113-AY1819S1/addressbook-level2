...
== Listing all person in sorted order by name : `sort`

Shows a list of all persons in sorted order by their name, along with their non-private details, in the address book. +
Format: `sort`
...
====
UPDATED: +
The search function is now not case-sensitive. +
User input will be forced to lowercase and searching in the set will also be forced to lowercase.  +
`find` will return you all substring that matches the keyword
