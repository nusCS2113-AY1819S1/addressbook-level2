...
Examples:
// inform user option if it is case insensitive or not
* `find john` +
Returns `John Doe` and/or `john`.

* `find betsy tim john` +
Returns Any person having names `Betsy`, `Tim`, or `John` and \ or Returns Any person having names `betsy`, `tim`, or `john`
