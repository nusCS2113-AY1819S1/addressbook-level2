...
== Finding all persons with their email: `find_Email`

Finds persons whose email matches the given keywords. +
Format: `find KEYWORD [MORE_KEYWORDS]`

[NOTE]
====
The search is case sensitive, the order of the keywords does not matter, only the email is searched,
and persons matching at least one keyword will be returned (i.e. `OR` search).
====

Examples:

* `find_Email John@abc.com` +
Returns `John Doe` but not `john`.

* `find_Email Betsy@abc.com Tim@xyz.com John@jju.bob` +
Returns Any person having emails `Betsy@abc.com`, `Tim@xyz.com`, or `John@jju.bob`.
