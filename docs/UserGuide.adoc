...
[NOTE]
====
The search do not need to be case sensitive, the order of the keywords does not matter, only the name is searched,
and persons matching at least one keyword will be returned (i.e. `OR` search). the find should allow  non case sensitive operation.
====
...
Returns `John Doe` .
* `find Betsy Tim John` +
Returns Any person having names `Betsy`, `Tim`, or `John` or `betsy`, `tim`, or `john`.
