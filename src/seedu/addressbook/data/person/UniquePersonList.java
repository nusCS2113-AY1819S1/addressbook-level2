...
public class UniquePersonList implements Iterable<Person> {
    ...
    /**
     * Returns an unmodifiable java List view with sorted elements cast as immutable {@link ReadOnlyPerson}s.
     * For use with other methods/libraries.
     * Any changes to the internal list/elements are immediately visible in the returned list.
     */
    public List<ReadOnlyPerson> sortedImmutableListView() {
        List<Person> sortedInternalList = internalList;
        sortedInternalList.sort(Comparator.comparing(p -> p.getName().fullName));

        return Collections.unmodifiableList(sortedInternalList);
    }
